...
class GridWorldMDP:
    ...
    def run_policy_iterations(self, discount=1.0, iterations=10):
        ...
        ### START CODE HERE ###
        utility_grid = np.zeros_like(self._reward_grid)
        policy_grid = np.zeros_like(self._reward_grid)
        for i in range(iterations):
            ...
            for t in range(10):
                for k in range(M):
                    for j in range(N):
                        out[k, j] = discount * np.sum(np.sum(self._T[k, j, int(policy_grid[k, j]), :, :] * utility_grid,
                            axis=-1), axis=-1) + self._reward_grid[(k,j)]
            utility_grid = out
            utility_grids[:, :, i] = out
            policy_grids[:, :, i] = self.best_policy(utility_grids[:, :, i])
            policy_grid = self.best_policy(utility_grids[:, :, i])
        ### END CODE HERE ###
